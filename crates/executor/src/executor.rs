...
mod tests {
    ...
    #[test]
    fn test_vol_executor_builder_new() {
        use std::sync::Arc;
        let storage = Arc::new(MockStorageForExecutor::new());
        let _builder = VolExecutorBuilder::new(storage);
    }

    #[test]
    fn test_vol_executor_builder_build_seq_scan() {
        use std::sync::Arc;
        let storage = Arc::new(
            MockStorageForExecutor::new().with_table("users", vec![vec![Value::Integer(1)]]),
        );
        let builder = VolExecutorBuilder::new(storage);
        let schema = Schema::new(vec![Field::new("id".to_string(), DataType::Integer)]);
        let plan = sqlrustgo_planner::SeqScanExec::new("users".to_string(), schema.clone());
        let result = builder.build(&plan);
        assert!(result.is_ok());
    }

    #[test]
    fn test_vol_executor_builder_unsupported_plan() {
        use std::sync::Arc;
        let storage = Arc::new(MockStorageForExecutor::new());
        let builder = VolExecutorBuilder::new(storage);
        struct DummyPlan {
        ...
        let result = builder.build(&DummyPlan::new());
        assert!(result.is_err());
    }

    #[test]
    fn test_mock_storage_for_executor_new() {
        let _storage = MockStorageForExecutor::new();
    }

    #[test]
    fn test_mock_storage_for_executor_with_table() {
        let storage =
            MockStorageForExecutor::new().with_table("users", vec![vec![Value::Integer(1)]]);
    ...
    #[test]
    fn test_mock_storage_for_executor_scan_empty() {
        let storage = MockStorageForExecutor::new();
    ...
    #[test]
    fn test_seq_scan_volcano_executor_all_rows_consumed() {
        use std::sync::Arc;
        let storage = Arc::new(MockStorageForExecutor::new().with_table(
            "test",
    ...
    #[test]
    fn test_seq_scan_volcano_executor_not_initialized() {
        use std::sync::Arc;
        let storage = Arc::new(MockStorageForExecutor::new());
        let schema = Schema::new(vec![Field::new("id".to_string(), DataType::Integer)]);
        let mut exec = SeqScanVolcanoExecutor::new("test".to_string(), schema, storage);
    ...
    #[test]
    fn test_limit_volcano_executor_zero_limit() {
        let child = Box::new(MockVolcanoExecutor::with_data(vec![
            vec![Value::Integer(1)],
            vec![Value::Integer(2)],
        ]));
        let schema = Schema::new(vec![Field::new("id".to_string(), DataType::Integer)]);
        let mut exec = LimitVolcanoExecutor::new(child, 0, 0, schema);
        exec.init().unwrap();
    ...
    #[test]
    fn test_limit_volcano_executor_offset_exceeds_rows() {
        let child = Box::new(MockVolcanoExecutor::with_data(vec![vec![Value::Integer(
            1,
        )]]));
        let schema = Schema::new(vec![Field::new("id".to_string(), DataType::Integer)]);
        let mut exec = LimitVolcanoExecutor::new(child, 10, 5, schema);
        exec.init().unwrap();
        let result = exec.next().unwrap();
        assert!(result.is_none());
    }
    ...
    #[test]
    fn test_limit_volcano_executor_name() {
        let child = Box::new(MockVolcanoExecutor::new());
        let schema = Schema::new(vec![Field::new("id".to_string(), DataType::Integer)]);
    ...
    #[test]
    fn test_limit_volcano_executor_not_initialized_error() {
        let child = Box::new(MockVolcanoExecutor::new());
        let schema = Schema::new(vec![Field::new("id".to_string(), DataType::Integer)]);
        let mut exec = LimitVolcanoExecutor::new(child, 10, 0, schema);
        let result = exec.next();
        assert!(result.is_err());
    }
    ...
    fn test_projection_volcano_executor_as_any() {
        let child = Box::new(MockVolcanoExecutor::new());
        let exec = ProjectionVolcanoExecutor::new(
            child,
            vec![],
            Schema::new(vec![Field::new("id".to_string(), DataType::Integer)]),
            Schema::new(vec![Field::new("id".to_string(), DataType::Integer)]),
