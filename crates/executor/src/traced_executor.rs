...
use crate::operator_profile::{OperatorProfile, QueryProfile, GLOBAL_PROFILER};
...
pub fn execute_with_trace(
    ...
    while let Some(row) = executor.next()? {
        rows.push(row);
        // Note: batches counted but not used in basic trace
    }
...
pub fn execute_with_trace_lightweight(
    ...
    while let Some(row) = executor.next()? {
        rows.push(row);
        // Note: batches counted but not used in lightweight trace
    }
...
// Tests are in executor.rs as they require access to MockVolcanoExecutor
