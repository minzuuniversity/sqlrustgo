...
# Cargo 安装
install_cargo() {
    log_info "开始使用 Cargo 安装 SQLRustGo v${VERSION}..."
...
# 源码编译安装
install_from_source() {
    log_info "开始从源码安装 SQLRustGo v${VERSION}..."
    ...
    if [ ! -d "$repo_dir" ]; then
        log_info "克隆仓库..."
        git clone --depth 1 --branch "rc/v1.4.0" "$REPO_URL" "$repo_dir" || {
            log_warn "版本 rc/v1.4.0 不存在，使用 release/v1.4.0..."
            git clone --depth 1 --branch "release/v1.4.0" "$REPO_URL" "$repo_dir" || {
                log_warn "使用 develop/v1.4.0 分支..."
                git clone --depth 1 --branch "develop/v1.4.0" "$REPO_URL" "$repo_dir"
            }
        }
    fi
